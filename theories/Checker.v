...
Section Indtypes.
  ...
  Definition infer_arities_inds (Σ : global_context) (ind_decl : minductive_decl) :
    ...
    List.fold_left (fun acc body =>
      ...
      | None => raise (BadInductive "Not enough products")
      | Some (params', _) => if forallb2 (fun '(na, ty) '(na', ty') =>
      eq_name na na' && eq_term ty ty') params params' then acc else
      raise (BadInductive "All inductive types must have syntactically equal parameters")
      end
    ) ind_decl.(ind_bodies) (ret params).

  Definition infer_constrs_inds (Σ : global_context) (Γ : context) (ind : inductive_body) : IndCheck () :=
